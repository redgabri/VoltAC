...
import com.volt.voltac.checks.impl.movement.NoSlowA;
...
public class PacketPlayerDigging extends PacketListenerAbstract {
    ...
    public static void handleUseItem(VoltPlayer player, ItemStack item, InteractionHand hand) {
        if (item == null) {
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_DIGGING) {
            ...
            if (dig.getAction() == DiggingAction.RELEASE_USE_ITEM) {
                final VoltPlayer player = VoltAPI.INSTANCE.getPlayerDataManager().getPlayer(event.getUser());
